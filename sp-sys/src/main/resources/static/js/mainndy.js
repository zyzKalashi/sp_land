...
function main() {
	...
	aA[aA.length - 1].onclick = aA[0].onclick = function() {
		if (bBtn) {
			...
			if (iNow == 1) {
				...
				aLi[aLi.length - 1].style.left = -aLi.length
						* aLi[0].offsetWidth + 'px';
			} else {
				iNow--;
			}
			iNow2--;
	...
	function toRun() {
		...
		startMove(oUl, {
			left : -(iNow2 - 1) * aLi[0].offsetWidth
		}, function() {
			if (iNow == 1) {
